...
def auto_categorize_blanks(df: pd.DataFrame, master_sku_df: pd.DataFrame = None) -> pd.DataFrame:
    """
    Automatically suggest categories for blank SKUs.
    
    Returns:
        DataFrame with additional columns: Suggested_Category, Confidence
    """
    results = []
    ...
    for idx, row in df.iterrows():
        ...
        results.append({
            ...
            'Current_Category': row.get('Product Category', ''),
...
def generate_category_report(suggestions_df: pd.DataFrame) -> str:
    ...
    report.append(f"HIGH CONFIDENCE ({len(high_conf)} items) - Review these first:")
    report.append("-" * 80)
    ...
    report.append(f"\nMEDIUM CONFIDENCE ({len(medium_conf)} items):")
    report.append("-" * 80)
    for _, row in medium_conf.iterrows():
        report.append(f"SKU: {row['SKU']} → {row['Suggested_Category']} ({row['Confidence']})")
...
if __name__ == "__main__":
    print("Vendor and category assignment utilities loaded.")
